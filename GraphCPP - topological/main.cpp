...
void TopologicalUtil(DirGraph<string>& g, string v, list<string> visited, list<string>& allVisited, stack<string>& sort)
{
	visited.push_back(v);
	...
	{
		if (find(visited.begin(), visited.end(), *it) == visited.end() && find(allVisited.begin(), allVisited.end(), *it) == allVisited.end())
		{
